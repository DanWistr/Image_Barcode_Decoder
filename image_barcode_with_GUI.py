...
IMAGE_PATH = 'C:/Users/Admin/Desktop/Neww/venv/sample_images_inverted/backside_clear_inverted.jpg'
boxed_dir = "C:/Users/Admin/Desktop/Neww/venv/sample_images_boxed"
...
class App(customtkinter.CTk):
    #function called by Detect Button
    ...
    #Creates Visible bounding boxes and texts for detected barcodes
    def draw_boxes_from_xml(self):
        if is_clicked == False:
            no_box_img = Image.open(IMAGE_PATH)
            # scale down to fit if needed
            w, h = no_box_img.size
            cap = 1450
            if w > cap:
                no_box_img = no_box_img.resize(
                    (cap, int(cap * h / w)),
                    resample=Image.Resampling.LANCZOS
                )
            self.photo = customtkinter.CTkImage(light_image=no_box_img, size=no_box_img.size)
            ...
            boxed_path = os.path.join(boxed_dir, filename_wo_ext + "_boxed.jpg")
    ...
    def __init__(self):
        ...
        self.decode_button.grid(row=0, column=0, padx=700, pady=10)
        ...
        self.success_button.grid(row=1, column=0, padx=700, pady=10)
